...
tax_grouped <- tax_slim %>% 
    ...
    summarise(
      n1_total = sum(n1),
      total_credit_amt = weighted.mean(total_credit_amt,p_total),
      taxable_income_amt = weighted.mean(taxable_income_amt,p_total),
      mortgageint_amt = weighted.mean(mortgageint_amt,p_total),
      p_mortgageint_nr = sum(mortgageint_nr),
      # netinvest_inc_amt  = weighted.mean(netinvest_inc_amt,p_total),
      # p_netinvest_inc_nr = sum(netinvest_inc_nr)/sum(n1),
      inctax_amt = weighted.mean(inctax_amt,p_total),
      p_unemploy_nr = sum(unemploy_nr)/sum(n1),
      # total_income_amt= weighted.mean(total_income_amt,p_total),
      agi_amt = weighted.mean(agi_amt,p_total),
      num_dependents = weighted.mean(num_dependents,p_total),
      re_taxes_nr = weighted.mean(re_taxes_nr,p_total),
...
```{r}
#TODO: Discuss with team which rows with NA we want to filter out based on features used
